...
rendered = {
    ...
    'YIELD': qb.reset().call("db.labels()").yield_(("label", "label")).where("label", "CONTAINS", "User").return_literal("count(label) AS numLabels"),
    'YIELD (list)': qb.reset().call("db.labels()").yield_([('length(labels)', 'len'), ('count(label)', 'cnt')]),
